...
const formatValue = (field, value) => {
  ...
  if (field === "expectedDate") return new Date(value).toLocaleDateString("es-ES");
  if (typeof value === "object") return value.name || value._id || "-";
  return value;
};

function PurchaseOrderHistoryModal(props) {
  const [history] = createResource(
    () => props.purchaseOrder,
    async (purchaseOrder) => {
      if (!purchaseOrder) return null;
      return purchaseOrdersApi.getHistory(purchaseOrder._id);
    },
  );
                      ...
                      <div class="flex justify-between items-start mb-3">
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {log.action === "create" && "✨ Orden creada"}
                            {log.action === "update" && "✏️ Orden actualizada"}
                          </p>
