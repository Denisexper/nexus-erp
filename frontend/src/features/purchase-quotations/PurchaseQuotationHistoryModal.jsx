...
const formatValue = (field, value) => {
  ...
  if (field === "validUntil") return new Date(value).toLocaleDateString("es-ES");
  if (typeof value === "object") return value.name || value._id || "-";
  return value;
};

function PurchaseQuotationHistoryModal(props) {
  const [history] = createResource(
    () => props.purchaseQuotation,
    async (purchaseQuotation) => {
      if (!purchaseQuotation) return null;
      return purchaseQuotationsApi.getHistory(purchaseQuotation._id);
    },
  );
                      ...
                      <div class="flex justify-between items-start mb-3">
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {log.action === "create" && "✨ Cotización registrada"}
                            {log.action === "update" && "✏️ Cotización actualizada"}
                          </p>
                            ...
                            {(field) => (
                              <Show when={log.dataAfter[field] !== undefined}>
                                <p class="text-sm text-gray-700 dark:text-gray-300">
                                  <span class="font-medium">{FIELD_LABELS[field]}:</span>{" "}
...
export default PurchaseQuotationHistoryModal;
