...
// branch/warehouse llegan poblados (objeto {name}) o como id crudo, según el
// snapshot; status usa las mismas etiquetas que la tabla/detalle.
const formatValue = (field, value) => {
  if (value === undefined || value === null || value === "") return "-";
  if (field === "status") return statusLabel(value);
  if (field === "requiredDate") return new Date(value).toLocaleDateString("es-ES");
  if (typeof value === "object") return value.name || value._id || "-";
  return value;
};

function PurchaseRequestHistoryModal(props) {
  const [history] = createResource(
    () => props.purchaseRequest,
    async (purchaseRequest) => {
      if (!purchaseRequest) return null;
      return purchaseRequestsApi.getHistory(purchaseRequest._id);
    },
  );
                      ...
                      <div class="flex justify-between items-start mb-3">
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {log.action === "create" && "✨ Solicitud creada"}
                            {log.action === "update" && "✏️ Solicitud actualizada"}
                          </p>
...
export default PurchaseRequestHistoryModal;
