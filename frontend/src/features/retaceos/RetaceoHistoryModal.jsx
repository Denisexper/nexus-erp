...
function RetaceoHistoryModal(props) {
  const [history] = createResource(
    () => props.retaceo,
    async (retaceo) => {
      if (!retaceo) return null;
      return retaceosApi.getHistory(retaceo._id);
    },
  );
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <Show when={history() && history().data}>
            <Show
              ...
              fallback={
                <div class="text-center py-8 text-gray-500 dark:text-gray-400">
                  No hay cambios registrados para este retaceo
                </div>
              }
            >
              <div class="space-y-4">
                      ...
                      <div class="flex justify-between items-start mb-3">
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {log.action === "create" && "✨ Retaceo registrado"}
                            {log.action === "update" && "✏️ Retaceo actualizado"}
                          </p>
...
export default RetaceoHistoryModal;
