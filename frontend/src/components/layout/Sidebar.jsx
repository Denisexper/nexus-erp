...
// El Sidebar se re-monta en cada navegación (cada página envuelve su propio
// <Layout>), así que un signal normal no alcanza para recordar el scroll ni
// qué grupos dejó colapsados: se reinicia cada vez. Estas dos variables viven
// a nivel de módulo, fuera del componente, así que sobreviven al remount
// mientras dure la sesión del SPA (se reinician solo con un F5).
let savedNavScroll = 0;
const collapsedSections = {};

function NavSection(props) {
  const [collapsed, setCollapsed] = createSignal(!!collapsedSections[props.id]);

  const toggle = () => {
    const next = !collapsed();
    collapsedSections[props.id] = next;
    setCollapsed(next);
  };
  ...
  return (
    <div class="pt-4 pb-1">
      <button
        type="button"
        onClick={toggle}
        class="w-full flex items-center justify-between px-4 py-1 text-xs font-semibold
               text-white/30 uppercase tracking-wider hover:text-white/50 transition-colors"
      >
...
function Sidebar() {
  ...
  onMount(() => {
    if (navRef) navRef.scrollTop = savedNavScroll;
  });
  ...
  return (
    <aside class="w-64 h-screen border-r border-black/20
      ...
      <nav
        ...
        class="flex-1 min-h-0 overflow-y-auto px-3 py-4 space-y-1"
      >
        ...
        <Show when={auth.hasPermission('companies.read') || auth.hasPermission('branches.view')}>
          <NavSection id="organizacion" title="Organización">
        ...
        <Show when={
          auth.hasPermission('warehouse_categories.view') ||
          auth.hasPermission('warehouses.view') ||
          auth.hasPermission('locations.view') ||
          auth.hasPermission('kardex.view')
        }>
          <NavSection id="almacenes" title="Almacenes">
        ...
        <Show when={auth.hasPermission('countries.view')}>
          <NavSection id="catalogo" title="Catálogo">
        ...
        <Show when={auth.hasPermission('expense_types.view') || auth.hasPermission('purchase_requests.view') || auth.hasPermission('purchase_quotations.view') || auth.hasPermission('purchase_orders.view') || auth.hasPermission('purchases.view') || auth.hasPermission('retaceos.view')}>
          <NavSection id="compras" title="Compras">
        ...
        <Show when={auth.hasPermission('users.read') || auth.hasPermission('users.create') || auth.hasPermission('users.update') || auth.hasPermission('roles.read') || auth.hasPermission('logs.read')}>
          <NavSection id="administracion" title="Administración">
